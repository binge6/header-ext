...
function normalizeCondition(raw: unknown): RuleCondition {
  ...
  if (typeof source.urlFilter === "string")
    condition.urlFilter = source.urlFilter;
  if (typeof source.useRegex === "boolean")
    condition.useRegex = source.useRegex;
  ...
  const resourceTypes = asStringArray(source.resourceTypes).filter(
    (item): item is ResourceType => (RESOURCE_TYPES as string[]).includes(item),
  );
